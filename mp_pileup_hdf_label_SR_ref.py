...
def feature_encode(pileup):
    ...
    pileup_counts = np.zeros(shape=(len(positions), num_features+2), dtype=np.float32)
    ref_info = None
    for i in range(len(positions)):
        ...
        pileup_counts[i, 10] = truth_coverage[ref_position] #wrs code
        if sr_coverage[ref_position] != 0.:
            pileup_counts[i, 11] = 1
...
def label_encode(pileup):
    ...
    for i in range(len(positions)):
        ...
        truth_base = truth[reference_pos].strip("^]").strip("$").upper()   #wrs code 加了.strip(",")
...
def pack_feature_encode(mpileup_file):
    ...
    for i in range(0, len(pileup_total) - 1):
        ...
        if not (np.any(truth)) and not (np.any(sr)):  # 如果全0则写出去
            cur_end = i + 1
            read = ''.join([e for e in sequence[cur_start: cur_end]])
            print("uncovered: ", seq_name[i])
            write_wrs(seq_name[i], read, parsed_args.uncovered_fasta)  # 将序列写出去
            read = ''
        else:
            cur_end = i + 1
            ...
            # print("covered: ", seq_name[i])
            # print(read)
            # write_wrs(seq_name[i], read, covered_file)  # 将序列写出去
            # read = ''
            # # test end
...
if __name__ == '__main__':
    ...
    # output_file = "/itmslhppc/itmsl0212/Projects/python/wrsCode/VariantWorks/data/wrs_sample/split_test/test_part.hdf"  #F.an E.coli
    # mpileup_folder = "/itmslhppc/itmsl0212/Projects/python/wrsCode/VariantWorks/data/wrs_sample/split_test/output_part"
    #
    # uncovered_fasta_folder = "/itmslhppc/itmsl0212/Projects/python/wrsCode/VariantWorks/data/wrs_sample/split_test/uncovered_part.fasta"
    # covered_file = "/itmslhppc/itmsl0212/Projects/python/wrsCode/VariantWorks/data/wrs_sample/split_test/covered_part.fasta"
    check_file(parsed_args.uncovered_fasta)  # clear the output file to avoid overlying
    # check_file(covered_file)  # clear the output file to avoid overlying
